"""core/patch6_report.py
    ...
    def _load_latest_runs(tickers: List[str], period_ref: str) -> pd.DataFrame:
        ...
        # Pega o último created_at por ticker para o period_ref informado
        q = text("""
            with ranked as (
                select
                    ticker,
    ...
    def _badge(texto: str, tone: str = "neutral") -> str:
        ...
        return f"<span style='display:inline-block;padding:2px 10px;border-radius:999px;border:1px solid {color};color:{color};font-weight:600;font-size:12px'>{texto}</span>"
    ...
    def render_patch6_report(
        ...
        if df_latest.empty:
            st.warning("Não há execuções salvas em patch6_runs para este period_ref e tickers do portfólio. Rode a LLM e salve os resultados primeiro.")
            return
...
"""
        ...
        view = df_latest.copy()
        view["sinal"] = view["perspectiva_compra"].fillna("").apply(lambda x: _badge(str(x).upper() if x else "—", _tone_from_perspectiva(str(x))))
        view = view[["ticker", "period_ref", "created_at", "sinal", "resumo"]].rename(
            columns={"created_at": "atualizado_em", "resumo": "resumo"}
        )
        st.markdown(
            view.to_html(escape=False, index=False),
        ...
        if show_company_details:
            ...
            for row in df_latest.itertuples(index=False):
                ...
                with st.expander(f"{tk}  {''}", expanded=False):
                    ...
                    # Template institucional (sem inventar nada)
                    st.markdown("**Tese (síntese)**")
                    st.write(row.resumo or "—")  # o resumo já vem do seu JSON

                    st.markdown("**Leitura / Direcionalidade**")
                    if p == "forte":
                        st.write("Viés construtivo, com sinais qualitativos favoráveis no recorte analisado. Mantém assimetria positiva, com monitoramento de riscos." )
                    elif p == "moderada":
                        st.write("Leitura equilibrada, com pontos positivos e ressalvas. Indica acompanhamento de gatilhos (execução, guidance, alocação de capital)." )
                    elif p == "fraca":
                        st.write("Leitura cautelosa, com sinais qualitativos desfavoráveis no recorte analisado. Recomenda postura defensiva e foco em mitigação de risco." )
                    else:
