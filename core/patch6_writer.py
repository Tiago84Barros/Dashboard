...
def normalize_result(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if score is None or _safe_int(score, 0) <= 0:
        # [SIGNAL] LLM não retornou score; usando heurística estrutural como fallback
        score = estimate_score(result)
        result["score_source"] = "heuristic"
        logger.warning("normalize_result: score_qualitativo ausente ou inválido — usando heurística (score=%s)", score)
    else:
        result["score_source"] = "llm"

    if confianca is None or _safe_float(confianca, 0.0) <= 0:
        # [SIGNAL] LLM não retornou confiança; usando heurística estrutural como fallback
        confianca = estimate_confidence(result)
        logger.warning("normalize_result: confianca_analise ausente ou inválida — usando heurística (confianca=%s)", confianca)
