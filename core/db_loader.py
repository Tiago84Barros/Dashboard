...
# Carrega múltiplos trimestrais (Cálculo para os últimos 12 mese) ---------------------------------------------------------------------------
@st.cache_data
...
# Carrega a tabela Demonstracoes_Completas (composta dos trimestres 3, 6, 9 e 12) ----------------------------------------------------------------------------------
@st.cache_data
def load_demonstracoes_completas_from_db(ticker: str) -> pd.DataFrame | None:
    """
    Carrega a tabela **Demonstracoes_Completas** para o ticker informado.
    Aceita `PETR4` ou `PETR4.SA` (faz tratamento interno).
    """
    path = _get_db_path()
    if path is None:
    ...
    query = (
        "SELECT * FROM Demonstracoes_Completas "
        f"WHERE Ticker = '{tk1}' OR Ticker = '{tk2}' "
        "ORDER BY Data ASC"
    )
    ...
    try:
        ...
        st.error(f"Erro ao carregar Demonstracoes_Completas para {ticker}: {e}")
        return None
