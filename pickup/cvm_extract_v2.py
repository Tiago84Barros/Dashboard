...
def collect_raw_rows() -> Tuple[List[dict], int]:
    ...
    log(f"Coletando {DOC_TYPE} RAW V2 do intervalo {ANO_INICIAL}..{last_year}")
    start = time.time()
    all_rows: List[dict] = []
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        for year in years:
            futures[executor.submit(process_year, session, year, ticker_map)] = year
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if year_errors:
                    failed_years[year] = year_errors
                    log(
                        f"{DOC_TYPE} {year}: falhou com {len(year_errors)} erro(s).",
                        level="ERROR",
                        year=year,
                        stage="year_failed",
                ...
                log(f"Falha no ano {year}: {exc}", level="ERROR", year=year, stage="future_failed")

    elapsed = round(time.time() - start, 1)
    log(f"[OK] Coleta raw concluída em {elapsed}s.")

    if failed_years:
        preview = {year: errs[:2] for year, errs in sorted(failed_years.items())}
        raise RuntimeError(f"Falhas em {len(failed_years)} ano(s) {DOC_TYPE}: {preview}")
    ...
    try:
        if _IngestionLog:
            with _IngestionLog(f"{DOC_TYPE.lower()}_raw_v2") as _log_ctx:
                ...
                rows, last_year = collect_raw_rows()
                _log_ctx.set_metric("ultimo_ano_disponivel", last_year)
                _log_ctx.set_metric("raw_rows", len(rows))
                ...
                _finish_run(
                    ...
                    status="success",
                    last_year=last_year,
                    metrics={"raw_rows": len(rows), "inserted_rows": inserted},
                )
        else:
            rows, last_year = collect_raw_rows()
            inserted = upsert_cvm_financial_raw(rows, run_id)
