...
try:
    ...
    # Patch 6 (CVM/IPE) — Direcionalidade Estratégica (overlay de aporte)
    try:
        from page.portfolio_patches import render_patch6_cvm_ipe_direcionalidade
    except Exception:
        render_patch6_cvm_ipe_direcionalidade = None  # type: ignore
...
def render():
    ...
    if empresas_lideres_finais:
        ...
        if render_patch6_cvm_ipe_direcionalidade is not None and empresas_lideres_finais:
            with st.expander("🧩 Patch 6 — Direcionalidade Estratégica (CVM/IPE)", expanded=False):
                try:
                    # Overlay: sugere percentuais de aporte (não troca ativos)
                    render_patch6_cvm_ipe_direcionalidade(
                        empresas_lideres_finais=empresas_lideres_finais,
                        score_global=score_global,
                        precos=df_prices_global,
                        aporte_mes=None,
