# =========================
# page/criacao_portfolio.py  (SEM persistência) - versão limpa (mantém anti-"zerar")
# =========================
...
def _render_all_patches(
    *,
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    # Nota: os "checkpoints" que você vê estão dentro desse patch (ajustaremos no portfolio_patches.py)
    render_patch7_validacao_evidencias(
        score_global=score_global,
        lideres_global=lideres_global,
        empresas_lideres_finais=empresas_lideres_finais,
        ...
        # se você aplicar o ajuste do item 2 abaixo, pode passar debug=False aqui
        # debug=False,
    )
...
def render():
    ...
    # snapshot leve para reexibir após rerun (evita "zerar")
    if "diag_last_result" not in st.session_state:
        st.session_state["diag_last_result"] = None  # type: ignore
    ...
    # reexibe último resultado se não clicou (modo "produção": sem botão extra)
    if not gerar:
        last = st.session_state.get("diag_last_result")
        if last:
            st.info("Reexibindo último resultado calculado nesta sessão (sem recalcular).")
            _render_bloco_final_portfolio(last["empresas_lideres_finais"])
            st.markdown("<hr>", unsafe_allow_html=True)
            _render_all_patches(
                score_global=last["score_global"],
                lideres_global=last["lideres_global"],
                empresas_lideres_finais=last["empresas_lideres_finais"],
                precos_patch5=last["precos_patch5"],
        ...
        for idx, ticker_col in enumerate(empresas_estrategia):
            ...
            col.markdown(
                ...
                <div style='border: 1px solid #ccc; border-radius: 8px; padding: 10px; margin-bottom: 10px; text-align: center;'>
                    ...
                    <p style='margin: 0; color: #999; font-size: 12px;'>{anos_lider_str}</p>
    ...
    # Patch 5: preços mínimos (tickers finais)
    ...
    # snapshot leve para reruns (não é persistência externa)
    st.session_state["diag_last_result"] = {
        "score_global": score_global,
        "lideres_global": lideres_global,
        "empresas_lideres_finais": empresas_lideres_finais,
        "precos_patch5": precos_patch5,
