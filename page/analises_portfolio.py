...
def render() -> None:
    ...
    def _get_temporal_chunks_for_ticker(t: str, top_k_used: int):
        recent, fonte_recent, mix_recent = _get_chunks_for_ticker(
            t,
            top_k_used=max(18, min(top_k_used, 40)),
            months_window=12,
            return_debug=True,
        )
        ...
        if int(analysis_window_months) <= 24:
            cumulative_24, fonte_24, mix_24 = _get_chunks_for_ticker(
                t,
                top_k_used=max(16, min(top_k_used, 36)),
                months_window=24,
                return_debug=True,
            )
            ...
            sections = {
                "janela_0_12m": recent[:24],
                "janela_12_24m": previous[:24],
                "janela_24_36m": [],
            }
        ...
        cumulative_36, fonte_36, mix_36 = _get_chunks_for_ticker(
            t,
            top_k_used=max(24, min(top_k_used, 54)),
            months_window=36,
            return_debug=True,
        )
        ...
        sections = {
            "janela_0_12m": recent[:24],
            "janela_12_24m": middle[:24],
            "janela_24_36m": older[:24],
        }
    ...
    def _build_prompt(contexto: str) -> str:
        return f"""
Você é um analista fundamentalista institucional, com postura de investidor disciplinado e foco em trajetória estratégica, execução, alocação de capital, margem de segurança, governança e consistência de discurso corporativo.
...
- não invente fatos;
- fundamente opiniões em fatos, números e eventos documentais;
- quando houver pouca base, diga isso explicitamente;
- use idealmente entre 8 e 12 evidências quando o contexto trouxer material suficiente;
- sempre que possível, atribua ano ou janela temporal às evidências;
- prefira evidências sobre dívida, capex, dividendos, recompra, guidance, execução, governança, M&A, emissões, covenants, contingências e mudanças materiais de estrutura de capital.
- em consideracoes_llm, emita um parecer profissional e opinativo, sem floreio, apontando se o caso parece disciplinado, oportunista, arriscado ou inconsistente do ponto de vista de um investidor fundamentalista.
...
{{
  ...
  "riscos": ["..."],
  "o_que_monitorar": ["..."],
  "consideracoes_llm": "parecer analítico final, com opinião fundamentada de investidor fundamentalista disciplinado, destacando margem de segurança, qualidade da alocação de capital, governança e previsibilidade dos fatos observados",
  "evidencias": [
    {{
    ...
    if st.button("🔄 Atualizar relatório com LLM agora"):
        ...
        for i, t in enumerate(tickers_run, start=1):
            ...
            try:
                ...
                contexto = _build_temporal_context(temporal_sections, per_chunk_chars=1250, total_chars=28000)
                ...
                try:
                    ...
                    if "context window" in msg or "exceed" in msg:
                        ...
                        contexto = _build_context_limited(flat_chunks, per_chunk_chars=900, total_chars=12000)
                        raw = _call_llm(client, _build_prompt(contexto))
                    else:
                        raise
