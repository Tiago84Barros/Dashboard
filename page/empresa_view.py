...
def render_header_empresa(nome: str | None, website: str | None, price: float | None, ticker: str, logo_url: str) -> None:
    # Header no estilo do print (texto à esquerda + logo grande em card à direita)
    st.markdown(
        f"""
        <div class="cf-header">
            <div>
                ...
                <p class="cf-subtitle">
                    <strong>{(nome or "-")}</strong> • {(website or "-")}
                    • Preço atual: <strong>{("R$ " + f"{price:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")) if price is not None else "-"}</strong>
                </p>
            </div>
...
def render_empresa_view(ticker: str):
    ...
    _inject_cf_css()
    render_header_empresa(company_name, company_website, current_price, ticker, logo_url)
