...
# Patch 6 - ingest & RAG (opcional, via pasta pickup)
try:
    from pickup.docs_rag import build_docs_by_ticker_from_db  # type: ignore
except Exception:
    build_docs_by_ticker_from_db = None  # type: ignore

try:
    from pickup import ingest_docs_cvm_ipe as _ingest_cvm_ipe  # type: ignore
except Exception:
    _ingest_cvm_ipe = None  # type: ignore
...
# ─────────────────────────────────────────────────────────────
# Patch 6 — ingest (CVM IPE) via módulo pickup.ingest_docs_cvm_ipe
# ─────────────────────────────────────────────────────────────

def _run_ingest_docs_cvm_ipe(
    tickers: List[str],
    *,
    years: str,
    tipos: str = "",
    max_docs_per_ticker: int = 25,
    chunk_size: int = 1200,
    overlap: int = 180,
) -> Dict[str, Any]:
    """Executa o coletor CVM/IPE do módulo pickup.ingest_docs_cvm_ipe (sem precisar rodar arquivo separado).

    Observação: o script pickup.ingest_docs_cvm_ipe foi escrito como CLI (argparse).
    Aqui, fazemos uma chamada programática ajustando sys.argv e rodando main().
    """
    if _ingest_cvm_ipe is None:
        raise RuntimeError("Módulo pickup.ingest_docs_cvm_ipe não encontrado (coloque ingest_docs_cvm_ipe.py em /pickup).")

    tks = sorted(list(dict.fromkeys([_norm_tk(t) for t in (tickers or []) if _norm_tk(t)])))
    if not tks:
    ...
    try:
        args = [
            ...
            "--tickers", ",".join(tks),
            "--years", str(years),
            "--max_docs_per_ticker", str(int(max_docs_per_ticker)),
            "--chunk_size", str(int(chunk_size)),
            "--overlap", str(int(overlap)),
        ]
        if tipos:
            args += ["--tipos", str(tipos)]
        _sys.argv = args
        t0 = time.time()
        _ingest_cvm_ipe.main()
        return {"ok": True, "tickers": tks, "seconds": round(time.time() - t0, 2)}
    finally:
        _sys.argv = argv0
...
def render_patch6_perspectivas_factibilidade(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    # (Opcional) Atualização automática de documentos oficiais (CVM/IPE)
    # - roda apenas sob clique (não dispara em rerun)
    ...
    with st.expander("⚙️ Atualizar base de documentos (CVM/IPE) — somente empresas selecionadas", expanded=False):
        st.caption(
            "Isso alimenta as tabelas docs_corporativos / docs_corporativos_chunks no Supabase, "
            "para o Patch 6 consumir trechos oficiais sem rodar arquivo separado."
        )

        if _ingest_cvm_ipe is None:
            st.warning("Módulo `pickup/ingest_docs_cvm_ipe.py` não encontrado. Coloque o arquivo na pasta pickup.")
        else:
            ano_atual = datetime.now().year
            default_years = f"{ano_atual-1}-{ano_atual}"
            c1, c2, c3 = st.columns([1.1, 1.1, 1.2])
            with c1:
                years = st.text_input("Anos (ex.: 2024-2026)", value=default_years, help="Janela de anos para buscar documentos na CVM/IPE.")
            with c2:
                max_docs = st.number_input("Máx. docs por ticker", min_value=5, max_value=200, value=25, step=5)
            with c3:
                tipos = st.text_input("Tipos (opcional, CSV)", value="", help="Ex.: ITR,DFP,FRE,COMUNICADO. Vazio = todos os tipos suportados.")
            c4, c5, c6 = st.columns([1.0, 1.0, 1.0])
            with c4:
                chunk_size = st.number_input("Chunk size", min_value=400, max_value=2400, value=1200, step=100)
            with c5:
                overlap = st.number_input("Overlap", min_value=0, max_value=600, value=180, step=20)
            with c6:
                run_btn = st.button("📥 Capturar/Atualizar docs (tickers selecionados)", use_container_width=True)

            if run_btn:
                with st.spinner("Coletando e persistindo documentos no Supabase…"):
                    try:
                        res = _run_ingest_docs_cvm_ipe(
                            tickers,
                            years=str(years).strip(),
                            tipos=str(tipos).strip(),
                            max_docs_per_ticker=int(max_docs),
                            chunk_size=int(chunk_size),
                            overlap=int(overlap),
                        )
                        st.success(f"OK: ingest finalizado para {len(res.get('tickers', []))} tickers em {res.get('seconds')}s.")
                    except Exception as e:
                        st.error(f"Falha no ingest: {type(e).__name__}: {e}")

    # ------------------------------------------------------------------
    # RAG automático via Supabase (docs_corporativos_chunks)
    # ------------------------------------------------------------------
    usar_docs_supabase = st.checkbox(
        "Usar docs do Supabase (docs_corporativos_chunks) automaticamente",
        value=True,
        help="Se marcado, o Patch 6 busca trechos relevantes no Supabase para cada ticker.",
    )

    docs_auto_map = None
    if usar_docs_supabase:
        if build_docs_by_ticker_from_db is None:
            st.warning("Função build_docs_by_ticker_from_db não encontrada em pickup/docs_rag.py.")
        else:
            with st.spinner("Montando RAG (trechos relevantes) no Supabase…"):
                try:
                    docs_auto_map = build_docs_by_ticker_from_db(
                        tickers=tickers,
                        top_k_per_ticker=8,
                        max_chars_per_chunk=1400,
                    )
                except Exception as e:
                    st.warning(f"RAG automático falhou: {type(e).__name__}: {e}")
                    docs_auto_map = None
    ...
    for tk in tickers:
        ...
        # Textos automáticos (via argumento docs_by_ticker) + (via Supabase RAG: docs_auto_map)
        def _append_docs(dmap: Optional[Dict[str, List[Dict[str, Any]]]], label_default: str) -> None:
            if not dmap:
                return
            items = dmap.get(tk) or []
            for it in items[: int(max_textos)]:
                if isinstance(it, dict) and str(it.get("text", "")).strip():
                    textos.append({
                        "source": str(it.get("source", label_default)).strip(),
                        "date": str(it.get("date", "NA")).strip(),
                        "text": str(it.get("text", "")).strip()
                    })

        _append_docs(docs_by_ticker, "auto")
        _append_docs(docs_auto_map, "supabase")
