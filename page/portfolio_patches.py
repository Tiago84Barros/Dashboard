...
# ─────────────────────────────────────────────────────────────
# PATCH 5 — Contribuição Marginal ao Retorno
# ─────────────────────────────────────────────────────────────

def render_patch5_contribuicao_marginal(contrib_globais: List[Dict]) -> None:
    st.markdown("## 🧩 Contribuição Marginal ao Retorno (por Segmento)")
    st.caption(
        "Mostra quais ativos carregaram o resultado (valor final, peso final e retorno no período) "
        "por segmento aprovado."
    )

    if not contrib_globais:
        st.info("Contribuição marginal indisponível: nada foi acumulado nesta execução.")
        return

    dfc = pd.DataFrame(contrib_globais).copy()
    if dfc.empty:
        st.info("Contribuição marginal indisponível: dataframe vazio.")
        return
    ...
    with col1:
        st.metric("Patrimônio final (segmento)", f"R$ {float(df_seg['final_segmento'].iloc[0]):,.2f}" if "final_segmento" in df_seg.columns else "—")
    with col2:
        st.metric("Diferença vs Selic (segmento)", f"{float(df_seg['diff_vs_selic_pct'].iloc[0]):.1f}%" if "diff_vs_selic_pct" in df_seg.columns else "—")
    with col3:
        top1 = df_seg.iloc[0]
        st.metric("Maior contribuinte", f"{top1.get('ticker','—')} ({float(top1.get('peso_final',0))*100:.1f}%)")

    df_show = df_seg.copy()
    if "peso_final" in df_show.columns:
        df_show["peso_final_%"] = df_show["peso_final"] * 100.0
    if "retorno_periodo" in df_show.columns:
    ...
    with st.expander("🌐 Visão agregada (todos os segmentos aprovados)", expanded=False):
        ...
        st.caption("Aqui, o peso_final_% é a contribuição agregada no total dos segmentos aprovados.")
