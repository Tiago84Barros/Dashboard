                   ...
                   # Vamos construir um DataFrame com o histórico completo de cada empresa selecionada
                    ...
                    if len(df_historico) == 0:
                        ...
                        if not df_resultados.empty:
                            ...
                            # Encontrar todas as empresas agrupadas por segmento, ordenadas pelo score
                            melhores_por_segmento = df_resultados.merge(empresas_filtradas, on='ticker', how='left')
                            melhores_por_segmento = melhores_por_segmento.sort_values(by=['SEGMENTO', 'score'], ascending=[True, False])
                        
                            # Exibir os resultados em um layout organizado
                            segmentos_unicos = melhores_por_segmento['SEGMENTO'].unique()
                        
                            for segmento in segmentos_unicos:
                                st.markdown(f"### Segmento: {segmento}")
                                
                                # Filtrar empresas do segmento atual
                                empresas_segmento = melhores_por_segmento[melhores_por_segmento['SEGMENTO'] == segmento]
                                
                                colunas = st.columns(3)  # Criar 3 colunas para disposição em blocos
                                for idx, row in empresas_segmento.iterrows():
                                    with colunas[idx % 3]:
                                        st.markdown(f"""
